...
async function updateAllSlotStatusDistributor() {
  try {
    console.log('🔄 slot_status 테이블의 모든 distributor 업데이트 시작...\n');

    // 1. 모든 고유한 customer_id 조회 (keyword 조건 없이)
    ...
    const uniqueCustomerIds = [
      ...new Set(allCustomers.map(c => c.customer_id)),
    ];
    ...
    for (const customerId of uniqueCustomerIds) {
      ...
      // slot_status 업데이트 (모든 레코드, keyword 조건 없이)
...
updateAllSlotStatusDistributor();
