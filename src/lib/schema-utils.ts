...
export async function diagnoseSchemaCacheIssues() {
  ...
  console.log(`user_profiles 테이블: ${results.userProfilesExists ? '✅' : '❌'}`);
...
export async function fixSchemaCacheIssues() {
  ...
  try {
    ...
    if (!results.customersExists || !results.usersExists || !results.userProfilesExists) {
      console.log('1. supabase-schema.sql을 Supabase SQL Editor에서 실행하세요');
      console.log('2. 테이블이 존재하는지 Supabase 대시보드에서 확인하세요');
    }
    
    if (!results.rlsOk) {
      console.log('3. RLS 정책을 확인하고 재설정하세요');
      console.log('4. Supabase 대시보드에서 Authentication > Policies 확인하세요');
    }
