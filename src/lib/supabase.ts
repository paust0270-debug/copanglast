...
export async function addDistributor(
  distributor: Omit<Distributor, 'id' | 'created_at'>
) {
  try {
    const { data, error } = await supabase
      .from('distributors')
...
export async function addCustomer(
  ...
  try {
    const usedSlot = await activateOneSlot(customer.name);
    if (usedSlot) {
...
export async function addSlot(
  ...
  const { data, error } = await supabase.from('slots').insert([slot]).select();
...
export async function updateSlotsStatusByCustomerId(
  customerId: string,
  status: 'active' | 'inactive' | 'paused'
) {
  const { data, error } = await supabase
...
export async function activateOneSlot(customerId: string) {
  ...
  try {
    ...
    const slotStatusData =
      slotsData?.map(slot => {
        ...
        const usedSlots = customerSlots.filter(
          s => s.status === 'active'
        ).length;
        ...
        return {
          ...
          registrationDate: slot.created_at
            ? new Date(slot.created_at).toISOString().split('T')[0]
            : '',
          expiryDate:
            slot.created_at && slot.usage_days
              ? new Date(
                  new Date(slot.created_at).getTime() +
                    slot.usage_days * 24 * 60 * 60 * 1000
                )
                  .toISOString()
                  .split('T')[0]
              : '',
...
export function withSchemaCacheFix<T extends unknown[], R>(
  fn: (...args: T) => Promise<R>
): (...args: T) => Promise<R> {
  return async (...args: T): Promise<R> => {
    try {
      ...
      if (
        ...
        (error as Error)?.message?.includes('table') ||
        (error as Error)?.message?.includes('schema')
      ) {
        await forceSchemaRefresh();
