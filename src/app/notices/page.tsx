  ...
  const handleDelete = async (noticeId: number, noticeTitle: string) => {
    if (confirm(`"${noticeTitle}" 공지사항을 삭제하시겠습니까?`)) {
      try {
        ...
        if (!response.ok) {
          throw new Error(`공지사항 삭제 실패: ${response.status} ${response.statusText}`);
        }
        ...
        alert(`공지사항 삭제 중 오류가 발생했습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
      }
    }
  };
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            {filteredNotices.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500 text-lg mb-2">등록된 공지사항이 없습니다</p>
                <p className="text-gray-400">첫 번째 공지사항을 작성해보세요!</p>
              </div>
            ) : (
