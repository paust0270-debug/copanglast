...
function SlotAddPageContent() {
  ...
  // 페이지 제목 상태 관리
  const [pageTitle, setPageTitle] = useState('알리');
  ...
  const loadCustomerSlotStatus = async (username: string) => {
    try {
      ...
      const response = await fetch(
        `/api/slot-aliexpress?customerId=${customerId}&username=${username}&name=${encodeURIComponent(nameParam)}&currentUser=${currentUsername}`
      );
  ...
  useEffect(() => {
    ...
    if (customerId && username) {
      setPageTitle(`알리 - ${customerSlotStatus.customerName || username}`);
      setListTitle(
        `${customerSlotStatus.customerName || username}님의 슬롯 등록 목록`
      );
    } else {
      setPageTitle('알리 - 관리자 모드');
  ...
  const loadCustomers = async () => {
    try {
      ...
      let apiUrl = '/api/slot-aliexpress?type=slot_status';
      if (customerId && username) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const slotStatusData = {
        ...
        usage_days: 30,
        status: '작동중',
        slot_type: normalizeSlotType(slotType) || '알리', // 항상 한글 버전으로 정규화
      };

      // slot_status 테이블에 저장
      const response = await fetch('/api/slot-aliexpress', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCustomer = async (id: number | undefined) => {
    ...
    if (
      ...
      try {
        ...
        const response = await fetch(
          `/api/slot-aliexpress/${customerToDelete.db_id}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
