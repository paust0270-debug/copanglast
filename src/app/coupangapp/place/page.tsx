...
function SlotAddPageContent() {
  ...
  // 페이지 제목 상태 관리
  const [pageTitle, setPageTitle] = useState('플레이스');
  ...
  const loadCustomerSlotStatus = async (username: string) => {
    try {
      ...
      const response = await fetch(
        `/api/slot-place?customerId=${customerId}&username=${username}&name=${encodeURIComponent(nameParam)}&currentUser=${currentUsername}`
      );
  ...
  useEffect(() => {
    ...
    if (customerId && username) {
      setPageTitle(`플레이스 - ${customerSlotStatus.customerName || username}`);
      setListTitle(
        `${customerSlotStatus.customerName || username}님의 슬롯 등록 목록`
      );
    } else {
      setPageTitle('플레이스 - 관리자 모드');
  ...
  const loadCustomers = async () => {
    try {
      ...
      let apiUrl = '/api/slot-place?type=slot_status';
      if (customerId && username) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const slotStatusData = {
        ...
        usage_days: 30,
        status: '작동중',
        slot_type: normalizeSlotType(slotType) || '플레이스', // 항상 한글 버전으로 정규화
      };

      // slot_status 테이블에 저장
      const response = await fetch('/api/slot-place', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCustomer = async (id: number | undefined) => {
    ...
    if (
      ...
      try {
        ...
        const response = await fetch(
          `/api/slot-place/${customerToDelete.db_id}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
