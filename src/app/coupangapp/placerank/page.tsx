...
function SlotAddPageContent() {
  ...
  // 페이지 제목 상태 관리
  const [pageTitle, setPageTitle] = useState('N플레이스순위체크');
  ...
  const loadCustomerSlotStatus = async (username: string) => {
    try {
      ...
      const response = await fetch(
        `/api/slot-placerank?customerId=${customerId}&username=${username}&name=${encodeURIComponent(nameParam)}&currentUser=${currentUsername}`
      );
  ...
  useEffect(() => {
    ...
    if (customerId && username) {
      setPageTitle(
        `N플레이스순위체크 - ${customerSlotStatus.customerName || username}`
      );
      setListTitle(
        `${customerSlotStatus.customerName || username}님의 슬롯 등록 목록`
      );
    } else {
      setPageTitle('N플레이스순위체크 - 관리자 모드');
  ...
  const loadCustomers = async () => {
    try {
      ...
      let apiUrl = '/api/slot-placerank?type=slot_status';
      if (customerId && username) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/slot-placerank', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCustomer = async (id: number | undefined) => {
    ...
    if (
      ...
      try {
        // slot_placerank 테이블에서 삭제 (실제 데이터베이스 ID 사용)
        const response = await fetch(
          `/api/slot-placerank/${customerToDelete.db_id}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
