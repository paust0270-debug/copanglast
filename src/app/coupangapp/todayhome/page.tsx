...
function SlotAddPageContent() {
  ...
  // 페이지 제목 상태 관리
  const [pageTitle, setPageTitle] = useState('오늘의집');
  ...
  const loadCustomerSlotStatus = async (username: string) => {
    try {
      ...
      const response = await fetch(
        `/api/slot-todayhome?customerId=${customerId}&username=${username}&name=${encodeURIComponent(nameParam)}&currentUser=${currentUsername}`
      );
  ...
  useEffect(() => {
    ...
    if (customerId && username) {
      setPageTitle(`오늘의집 - ${customerSlotStatus.customerName || username}`);
      setListTitle(
        `${customerSlotStatus.customerName || username}님의 슬롯 등록 목록`
      );
    } else {
      setPageTitle('오늘의집 - 관리자 모드');
  ...
  const loadCustomers = async () => {
    try {
      ...
      let apiUrl = '/api/slot-todayhome?type=slot_status';
      if (customerId && username) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // slot_status 테이블에 저장
      const response = await fetch('/api/slot-todayhome', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCustomer = async (id: number | undefined) => {
    ...
    if (
      ...
      try {
        ...
        const response = await fetch(
          `/api/slot-todayhome/${customerToDelete.db_id}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
