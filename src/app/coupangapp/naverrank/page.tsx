...
function SlotAddPageContent() {
  ...
  // 페이지 제목 상태 관리
  const [pageTitle, setPageTitle] = useState('N쇼핑순위체크');
  ...
  const loadCustomerSlotStatus = async (username: string) => {
    try {
      ...
      const response = await fetch(
        `/api/slot-naverrank?customerId=${customerId}&username=${username}&name=${encodeURIComponent(nameParam)}&currentUser=${currentUsername}`
      );
  ...
  useEffect(() => {
    ...
    if (customerId && username) {
      setPageTitle(
        `N쇼핑순위체크 - ${customerSlotStatus.customerName || username}`
      );
      setListTitle(
        `${customerSlotStatus.customerName || username}님의 슬롯 등록 목록`
      );
    } else {
      setPageTitle('N쇼핑순위체크 - 관리자 모드');
  ...
  const loadCustomers = async () => {
    try {
      ...
      let apiUrl = '/api/slot-naverrank?type=slot_status';
      if (customerId && username) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const slotStatusData = {
        ...
        usage_days: 30,
        status: '작동중',
        slot_type: normalizeSlotType(slotType) || 'N쇼핑순위체크', // 항상 한글 버전으로 정규화
      };

      // slot_naverrank 테이블에 저장
      const response = await fetch('/api/slot-naverrank', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCustomer = async (id: number | undefined) => {
    ...
    if (
      ...
      try {
        // slot_naverrank 테이블에서 삭제 (실제 데이터베이스 ID 사용)
        const response = await fetch(
          `/api/slot-naverrank/${customerToDelete.db_id}`,
          {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
