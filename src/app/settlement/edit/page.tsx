...
function SettlementEditPageContent() {
    ...
    try {
      ...
      if (settlementId) {
        ...
        const response = await fetch(
          `/api/settlement-requests/edit/${settlementId}`
        );
        ...
        if (result.success) {
          ...
          const items = (result.data || []).map((item: any, index: number) => ({
            ...
            slot_addition_date: item.created_at
              ...
              : new Date().toISOString().split('T')[0], // 슬롯추가일을 created_at에서 가져오기
            category:
          ...
          if (result.settlementInfo) {
            ...
            setDepositDate(
              info.deposit_date || new Date().toISOString().split('T')[0]
            );
            setMemo(info.memo || '');
          ...
          console.log(
            '임시 데이터로 총입금액 테스트:',
            mockData.reduce((sum, item) => sum + item.payment_amount, 0)
          );
        }
      } else {
        // 정산 ID가 없으면 빈 데이터
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `정산 수정 API 요청 실패: ${response.status} ${response.statusText}`
        );
      }
      ...
      if (!result.success) {
        throw new Error(
          result.error || '정산 수정 처리 중 오류가 발생했습니다.'
        );
      }
      ...
      alert(
        `정산수정 처리 중 오류가 발생했습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
      );
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            {settlementId && (
              <p className="text-sm text-blue-600">정산 ID: {settlementId}</p>
            )}
          </div>
