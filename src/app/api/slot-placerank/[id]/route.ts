...
export async function DELETE(
  ...
  try {
    ...
    console.log(`🗑️ N플레이스순위체크 슬롯 삭제 요청 - ID: ${id}`);
    ...
    console.log(
      `✅ N플레이스순위체크 슬롯 초기화 완료 - ID: ${id}, 날짜 정보 보존됨`
    );
    ...
    return NextResponse.json({
      ...
      message:
        'N플레이스순위체크 슬롯이 성공적으로 초기화되었습니다. (날짜 정보 보존)',
    });
  } catch (error) {
    console.error('N플레이스순위체크 슬롯 초기화 API 예외 발생:', error);
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    console.log(`🔍 N플레이스순위체크 슬롯 조회 요청 - ID: ${id}`);
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: 'N플레이스순위체크 슬롯 조회 성공',
    });
  } catch (error) {
    console.error('N플레이스순위체크 슬롯 조회 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
...
export async function PUT(
  ...
  try {
    ...
    console.log(`✏️ N플레이스순위체크 슬롯 수정 요청 - ID: ${id}`, body);
    ...
    console.log(`✅ N플레이스순위체크 슬롯 수정 완료 - ID: ${id}`);
        ...
        if (findError) {
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              ...
              slot_type:
                normalizeSlotType(data.slot_type) || 'N플레이스순위체크', // 항상 한글 버전으로 정규화
            })
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: 'N플레이스순위체크 슬롯이 성공적으로 수정되었습니다.',
    });
  } catch (error) {
    console.error('N플레이스순위체크 슬롯 수정 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
