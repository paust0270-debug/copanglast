  ...
  try {
    ...
    if (type === 'slot_status') {
      ...
      let slotStatusQuery = supabase
        ...
        .in('slot_type', getSlotTypeVariants('N쇼핑순위체크')) // N쇼핑순위체크 슬롯 타입 (영문/한글 모두 포함) // N쇼핑순위체크 슬롯 타입만 필터링
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: availableSlots, error: slotsError } = await supabase
      ...
      .in('slot_type', getSlotTypeVariants('N쇼핑순위체크')) // N쇼핑순위체크 슬롯 타입 (영문/한글 모두 포함) // N쇼핑순위체크 슬롯 타입만 조회
    ...
    for (let i = 0; i < requestedSlotCount; i++) {
      ...
      const updateData = {
        ...
        memo: body.memo || '',
        slot_type: normalizeSlotType(body.slot_type) || 'N쇼핑순위체크', // 항상 한글 버전으로 정규화
