...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'slot_status') {
      // slot_coupangvip 테이블 조회 (쿠팡VIP 앱 추가 페이지용) - 키워드가 있는 레코드만
      let slotStatusQuery = supabase
        .from('slot_coupangvip')
      ...
      if (customerId && username) {
        ...
        console.log('🔍 개별 고객 쿠팡VIP 슬롯 필터링:', {
          customerId,
          username,
        });
      }
      ...
      if (slotStatusError) {
        console.error('slot_coupangvip 데이터 조회 오류:', slotStatusError);
        // 테이블이 없을 때는 빈 배열 반환
        if (
          slotStatusError.code === 'PGRST116' ||
          slotStatusError.message?.includes(
            'relation "slot_coupangvip" does not exist'
          )
        ) {
          console.log(
            '⚠️ slot_coupangvip 테이블이 존재하지 않습니다. 빈 배열을 반환합니다.'
          );
        ...
        return NextResponse.json(
          {
            error:
              '쿠팡VIP 슬롯 등록 데이터를 불러오는 중 오류가 발생했습니다.',
          },
          { status: 500 }
        );
      }
      ...
      const formattedSlotStatusData = slotStatusData?.map((slot, index) => {
        ...
        if (
          ...
          return null; // coupangapp/vip 페이지에서는 일시중지된 슬롯 제외
        }

        console.log('쿠팡VIP 슬롯 매칭 확인:', {
          slot_coupangvip_id: slot.id,
          slot_coupangvip_slot_sequence: slot.slot_sequence,
          slots_id: matchingSlot?.id,
          slot_coupangvip_usage_days: slot.usage_days,
          slots_usage_days: matchingSlot?.usage_days,
          slot_coupangvip_created_at: slot.created_at,
    ...
    // 기본: slots 테이블 조회 (슬롯 현황 페이지용) - 쿠팡VIP 슬롯 타입만
    ...
    if (customerId && username) {
      console.log('🔍 특정 고객 쿠팡VIP 슬롯 현황 조회:', {
        customerId,
        username,
      });
      ...
      console.log('📊 고객 쿠팡VIP 슬롯 현황:', customerSlots);

      // 1. 모든 slot_coupangvip 데이터를 한 번에 조회 (성능 최적화)
      ...
      console.log('📊 쿠팡VIP slot_sequence별 통계:', slotSequenceStats);
        ...
        data: [
          {
            ...
            slotType: customerSlots[0]?.slot_type || '쿠팡VIP',
    ...
    let filteredData =
      slotsData?.map(slot => {
        ...
        return {
          ...
          slotType: slot.slot_type || '쿠팡VIP',
          slotCount: slot.slot_count || 1,
          usedSlots: 0, // slot_coupangvip 테이블에서 계산
...
export async function POST(request: NextRequest) {
  try {
    console.log('🔄 쿠팡VIP 개별 슬롯 할당 처리 중...');
    ...
    const { data: allCustomerSlots, error: allSlotsError } = await supabase
      .from('slots')
      .select('id, customer_id, slot_type, slot_count, status')
      .eq('customer_id', customerId); // POST에서는 customerId 사용

    console.log('🔍 해당 고객의 모든 슬롯:', allCustomerSlots);
    console.log(
      '🔍 쿠팡VIP 슬롯 필터링:',
      allCustomerSlots?.filter(slot => slot.slot_type === '쿠팡VIP')
    );

    // 1. slots 테이블에서 해당 고객의 쿠팡VIP 슬롯만 조회 (usage_days 내림차순)
    ...
    try {
      ...
      if (
        error &&
        (error.code === 'PGRST116' ||
          error.message?.includes('relation "slot_coupangvip" does not exist'))
      ) {
        console.log(
          '⚠️ slot_coupangvip 테이블이 존재하지 않습니다. 빈 배열을 사용합니다.'
        );
        currentSlotStatus = [];
      } else if (error) {
        console.error('slot_coupangvip 조회 오류:', error);
        currentSlotStatus = [];
      } else {
        currentSlotStatus = data || [];
      }
    } catch (err) {
      console.error('slot_coupangvip 조회 예외:', err);
      currentSlotStatus = [];
    }
    ...
    // 2. 기존 slot_coupangvip 레코드 업데이트 (새 레코드 생성 방지)
    console.log('🔄 기존 slot_coupangvip 레코드 업데이트 시작...');
    ...
    if (
      emptySlotError &&
      (emptySlotError.code === 'PGRST116' ||
        emptySlotError.message?.includes(
          'relation "slot_coupangvip" does not exist'
        ))
    ) {
      console.log(
        '⚠️ slot_coupangvip 테이블이 존재하지 않습니다. 슬롯 등록을 위해 테이블을 생성해야 합니다.'
      );
      return NextResponse.json(
        {
          error:
            '쿠팡VIP 슬롯 테이블이 존재하지 않습니다. 관리자에게 문의하세요.',
        },
        { status: 500 }
      );
    } else if (emptySlotError) {
      console.error('빈 slot_coupangvip 레코드 조회 오류:', emptySlotError);
      return NextResponse.json(
    ...
    console.log(`📝 ${requestedSlotCount}개 쿠팡VIP 레코드 업데이트 중...`);
    ...
    updateResults.forEach((result, index) => {
      if (result.error) {
        console.error(
          `쿠팡VIP 레코드 ${index + 1} 업데이트 실패:`,
          result.error
        );
        errors.push(result.error.message || 'Unknown error');
      } else {
        successCount++;
        console.log(`✅ 쿠팡VIP 레코드 ${index + 1} 업데이트 성공`);
      }
    });

    if (successCount === 0) {
      return NextResponse.json(
        { error: '모든 쿠팡VIP 슬롯 업데이트에 실패했습니다.' },
        { status: 500 }
      );
    }

    console.log(
      `✅ ${successCount}/${requestedSlotCount}개 쿠팡VIP 레코드 업데이트 성공`
    );
    ...
    if (selectError) {
      console.error('업데이트된 쿠팡VIP 레코드 조회 오류:', selectError);
    }
    ...
    if (body.keyword && updatedSlotStatus && updatedSlotStatus.length > 0) {
      try {
        ...
        const keywordRecords = updatedSlotStatus.map(slot => ({
          ...
          slot_id: slot.id, // slot_coupangvip 레코드 ID를 slot_id로 사용
        }));

        console.log(
          `📝 ${keywordRecords.length}개 쿠팡VIP 키워드 레코드 생성 중...`
        );
        ...
        if (keywordError) {
          ...
          console.log(
            `✅ keywords 테이블에 ${keywordRecords.length}개 쿠팡VIP 레코드 삽입 성공`
          );
        }
        ...
        console.log(
          `📝 ${trafficRecords.length}개 쿠팡VIP traffic 레코드 생성 중...`
        );
        ...
        if (trafficError) {
          ...
          console.log(
            `✅ traffic 테이블에 ${trafficRecords.length}개 쿠팡VIP 레코드 삽입 성공`
          );
        }
      } catch (err) {
        console.error('쿠팡VIP keywords 테이블 처리 중 오류:', err);
    ...
    return NextResponse.json({
      ...
      message: `${requestedSlotCount}개 쿠팡VIP 슬롯이 성공적으로 등록되었습니다.`,
    });
  } catch (error) {
    console.error('쿠팡VIP 슬롯 등록 API 예외 발생:', error);
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
