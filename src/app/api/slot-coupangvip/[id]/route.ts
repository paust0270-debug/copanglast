...
export async function DELETE(
  ...
  try {
    ...
    console.log(`🗑️ 쿠팡VIP 슬롯 삭제 요청 - ID: ${id}`);
    ...
    if (fetchError) {
      console.error('삭제할 쿠팡VIP 슬롯 조회 오류:', fetchError);
      return NextResponse.json(
        { success: false, error: '삭제할 쿠팡VIP 슬롯을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }

    console.log(`📋 삭제할 쿠팡VIP 슬롯 정보:`, slotInfo);
    ...
    console.log('🔄 쿠팡VIP 슬롯 초기화 중 (날짜 정보 보존)...');
    ...
    if (resetError) {
      console.error('쿠팡VIP 슬롯 초기화 오류:', resetError);
      return NextResponse.json(
        {
          success: false,
          error: `쿠팡VIP 슬롯 초기화 중 오류가 발생했습니다: ${resetError.message}`,
        },
        { status: 500 }
      );
    }

    console.log(`✅ 쿠팡VIP 슬롯 초기화 완료 - ID: ${id}, 날짜 정보 보존됨`);

    // keywords 테이블에서 해당 키워드 정리
    try {
      console.log('🔄 keywords 테이블에서 쿠팡VIP 키워드 정리 중...');
      ...
      if (slotInfo.keyword) {
        ...
        if (findError) {
          ...
          if (deleteError) {
            ...
            console.log(
              `✅ keywords 테이블에서 쿠팡VIP 키워드 정리 완료: ${slotInfo.keyword}`
            );
          }
        }
      }
      ...
      console.log('⚠️ keywords 정리 실패했지만 쿠팡VIP 슬롯 초기화는 성공');
    }

    // traffic 테이블에서도 해당 키워드 정리
    try {
      console.log('🔄 traffic 테이블에서 쿠팡VIP 키워드 정리 중...');
      ...
      if (slotInfo.keyword) {
        ...
        if (findError) {
          ...
          if (deleteError) {
            ...
            console.log(
              `✅ traffic 테이블에서 쿠팡VIP 키워드 정리 완료: ${slotInfo.keyword}`
            );
          }
        }
      }
      ...
      console.log('⚠️ traffic 정리 실패했지만 쿠팡VIP 슬롯 초기화는 성공');
    }
    ...
    return NextResponse.json({
      ...
      message: '쿠팡VIP 슬롯이 성공적으로 초기화되었습니다. (날짜 정보 보존)',
    });
  } catch (error) {
    console.error('쿠팡VIP 슬롯 초기화 API 예외 발생:', error);
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    console.log(`🔍 쿠팡VIP 슬롯 조회 요청 - ID: ${id}`);
    ...
    if (error) {
      console.error('쿠팡VIP 슬롯 조회 오류:', error);
      return NextResponse.json(
        { success: false, error: '쿠팡VIP 슬롯을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: '쿠팡VIP 슬롯 조회 성공',
    });
  } catch (error) {
    console.error('쿠팡VIP 슬롯 조회 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
...
export async function PUT(
  ...
  try {
    ...
    console.log(`✏️ 쿠팡VIP 슬롯 수정 요청 - ID: ${id}`, body);
    ...
    if (fetchError) {
      console.error('기존 쿠팡VIP 데이터 조회 오류:', fetchError);
      return NextResponse.json(
        {
          success: false,
          error: '기존 쿠팡VIP 슬롯 데이터를 찾을 수 없습니다.',
        },
        { status: 404 }
      );
    }
    ...
    if (error) {
      console.error('쿠팡VIP 슬롯 수정 오류:', error);
      return NextResponse.json(
        {
          success: false,
          error: `쿠팡VIP 슬롯 수정 중 오류가 발생했습니다: ${error.message}`,
        },
        { status: 500 }
      );
    }

    console.log(`✅ 쿠팡VIP 슬롯 수정 완료 - ID: ${id}`);

    // keywords 테이블 동기화 (키워드나 링크가 변경된 경우)
    if (body.keyword || body.link_url) {
      try {
        console.log('🔄 keywords 테이블 쿠팡VIP 동기화 중...');
        ...
        if (findError) {
          console.error('기존 쿠팡VIP 키워드 조회 오류:', findError);
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              ...
              slot_type: normalizeSlotType('쿠팡VIP'), // 항상 한글 버전으로 정규화
            })
            .eq('id', existingKeywords[0].id);

          if (updateError) {
            console.error(
              'keywords 테이블 쿠팡VIP 업데이트 오류:',
              updateError
            );
          } else {
            console.log('✅ keywords 테이블 쿠팡VIP 동기화 완료');
          }
        } else {
          ...
          if (insertError) {
            console.error('keywords 테이블 쿠팡VIP 삽입 오류:', insertError);
          } else {
            console.log('✅ keywords 테이블에 새 쿠팡VIP 키워드 추가 완료');
          }
        }
      } catch (keywordError) {
        console.error('keywords 테이블 쿠팡VIP 동기화 예외:', keywordError);
        console.log('⚠️ keywords 동기화 실패했지만 쿠팡VIP 슬롯 수정은 성공');
      }
    }
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: '쿠팡VIP 슬롯이 성공적으로 수정되었습니다.',
    });
  } catch (error) {
    console.error('쿠팡VIP 슬롯 수정 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
