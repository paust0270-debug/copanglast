...
export async function DELETE(
  ...
  try {
    ...
    console.log(`🗑️ 알리엑스프레스 슬롯 삭제 요청 - ID: ${id}`);
    ...
    const resetData = {
      ...
      status: '작동중',
      memo: '',
      slot_type: normalizeSlotType('알리'), // 항상 한글 버전으로 정규화
    ...
    return NextResponse.json({
      ...
      message:
        '알리엑스프레스 슬롯이 성공적으로 초기화되었습니다. (날짜 정보 보존)',
    });
  } catch (error) {
    console.error('알리엑스프레스 슬롯 초기화 API 예외 발생:', error);
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    console.log(`🔍 알리엑스프레스 슬롯 조회 요청 - ID: ${id}`);
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: '알리엑스프레스 슬롯 조회 성공',
    });
  } catch (error) {
    console.error('알리엑스프레스 슬롯 조회 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
...
export async function PUT(
  ...
  try {
    ...
    console.log(`✏️ 알리엑스프레스 슬롯 수정 요청 - ID: ${id}`, body);
        ...
        if (findError) {
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              ...
              slot_type: normalizeSlotType(data.slot_type) || '알리', // 항상 한글 버전으로 정규화
            })
    ...
    return NextResponse.json({
      success: true,
      data: data,
      message: '알리엑스프레스 슬롯이 성공적으로 수정되었습니다.',
    });
  } catch (error) {
    console.error('알리엑스프레스 슬롯 수정 API 예외 발생:', error);
    return NextResponse.json(
      { success: false, error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
